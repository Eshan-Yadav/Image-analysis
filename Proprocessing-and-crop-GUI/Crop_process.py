...
invert_contour=0
crop_img=resized_image.copy()
def draw_contour(im=img,*args):
    global crop_img
    global resized_image
    def contour_selection(event, x, y, flags, param):
        global crop_img
        global resized_image
        selected_contour = None

        if event == cv.EVENT_LBUTTONDOWN:
            # Check if the clicked point is inside any contour
            color_img = resized_image.copy()
            color_img_invert = resized_image.copy()

            for contour_idx, contour in enumerate(contours):
                if cv.pointPolygonTest(contour, (x, y), False) >= 0:
                    selected_contour = contour_idx
                    crop_img = resized_image.copy()
                    
                    cv.fillPoly(crop_img, [contour], (0, 0, 0))
                    cv.imwrite("contour.tif",crop_img)

                    break
        
            
            # Display only the selected contour and hide the rest
            if selected_contour is not None:
                for contour_idx, contour in enumerate(contours):
                    if contour_idx == selected_contour:
                        cv.drawContours(color_img, [contour], -1, (0, 255, 0), 2)
                        cv.fillPoly(color_img, [contour], (0, 0, 0))

                    else:
                        cv.drawContours(color_img, [contour], -1, (0, 0, 0), 2)
            
            for h in range(color_img_invert.shape[0]):
                for w in range(color_img_invert.shape[1]):
                    is_inside = cv.pointPolygonTest(contour, (w, h), False)>=0
                    if not is_inside:
                        color_img_invert[h,w]=(0,0,0)
                
            # Display the image with the selected contour
            cv.imwrite("contour_inverted.tif",color_img_invert)
            cv.imshow('Cropper', color_img)
            cv.imshow('contour_inverted', color_img_invert)
    ...
    # cv.namedWindow('Image select')
    cv.imshow("contour",contour_img)
    cv.setMouseCallback('contour', contour_selection)
...
def save_contour():
    ...
    print("contour saved")
#save button
save_button_contour=tk.Button(contour_display,text="Save",command=save_contour)
save_button_contour.grid(row=2,column=0,padx=10,pady=10)
